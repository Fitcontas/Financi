var App = function() {
    ...
    function toggleSideBar(_this) {
        ...
        var w = $("#cl-wrapper");
        var s = $(".cl-sidebar");
        var user = $('.navbar-user');
        var navblock = $('.cl-navblock');

        if (w.hasClass("sb-collapsed")) {
            $(".fa", b).addClass("fa-angle-left").removeClass("fa-angle-right");
            $("#sidebar-collapse").removeClass('collapsed');
            w.removeClass("sb-collapsed");
            user.removeClass('sidebar-collapsed');
            navblock.css('margin-top', '131px');
            w.find('.fa-angle-down').removeClass('hide');
            w.find('.toggle-icon').addClass('hide').css('font-size', '17px');;
        } else {
            $(".fa", b).removeClass("fa-angle-left").addClass("fa-angle-right");
            $("#sidebar-collapse").addClass('collapsed');
            w.addClass("sb-collapsed");
            user.addClass('sidebar-collapsed');
            navblock.css('margin-top', '133px');
            w.find('.fa-angle-down').addClass('hide');
            w.find('.toggle-icon').removeClass('hide').css('font-size', '20px');
        }
    }
        ...
        init: function(options) {
            ...
            function showMenu(_this, e) {
                ...
                if (($("#cl-wrapper").hasClass("sb-collapsed") || ($(window).width() > 755 && $(window).width() < 963)) && $("ul", _this).length > 0) {
                    ...
                    var bottom = $(_this).offset().bottom;
                    ...
                    var height = ($('#sub-menu-nav').height() / 3) * 1.5;

                    tool.css('top', top - height);
...
$(function() {
    ...
    //Iniciando o App de efeitos e eventos do layouts.
    App.init();
