...
(function ($) {
    ...
    $.fn.columnFilter = function (options) {
        ...
        function fnCreateRangeInput(oTable, oSelect, aData) {
			...
			*	Update Date: 27th Oct, 2011			
            */
			
			if(oSelect == "")
			{
				from = $('<input type="text" class="number_range_filter" id="' + sFromId + '" rel="' + i + '"/>');
			}
			else
			{
				from = '<select class="number_range_filter" id="' + sFromId + '" rel="' + i + '"><option value="'+aData[0]+'">ALL</option>';
				...
				for (j = 0; j < iLen; j++) 
				{
					...
					{
						from += '<option value="' + escape(aData[j]) + '">' + aData[j] + '</option>';
					}
					else 
					{
						from += '<option value="' + escape(aData[j].value) + '">' + aData[j].label + '</option>';
					}
				}
			from += '</select>';
            }
			...
			if(oSelect == "")
			{
				to = $('<input type="text" class="number_range_filter" id="' + sToId + '" rel="' + i + '"/>');
			}
			else
			{
				to = '<select class="number_range_filter" id="' + sToId + '" rel="' + i + '"><option value="'+ aData[aData.length-1] +'">ALL</option>';
				var j = 0;
				var iLen = aData.length;
				for (j = 0; j < iLen; j++) 
				{
					if (typeof (aData[j]) != 'object') 
					{
						to += '<option value="' + escape(aData[j]) + '">' + aData[j] + '</option>';
					}
					else 
					{
						to += '<option value="' + escape(aData[j].value) + '">' + aData[j].label + '</option>';
					}
				}
				to += '</select>';
            }
...
*/
        ...
        return this.each(function () {
			...
			/* 	Updated the case statement to take care of new parameters added for range filter
            *	Updated By: 	Muhammad Waseem Arfi
			*	Update Date: 27th Oct, 2011
            */
			...
			*/
			...
			/* 	Added the case statement to take care of new filed type added
            *	Updated By: 	Muhammad Waseem Arfi
			*	Update Date: 27th Oct, 2011
            */
