...
class ioMenuItem implements \ArrayAccess, \Countable, \IteratorAggregate
{
  ...
  {
    return $this->show() && $this->checkUserAccess();
  }
  ...
  protected function _createChild($name, $route = null, $attributes = array(), $class = null)
  {
    ...
    return new self($name, $route, $attributes);
  }
  ...
  public function render($depth = null, $renderAsChild = false)
  {
    ...
    $html = $this->_format('<ul'.$this->_tag_options($attributes).'>', 'ul');
  ...
  public function renderChild($depth = null)
  {
    ...
    // opening li tag
    $html = $this->_format('<li'.$this->_tag_options($attributes).'>', 'li');
  ...
  public function renderLink()
  {
    ...
    if ($route == '' || $route == '#')
    {
      ...
      return $this->content_tag('a', $this->renderLabel(), $params);
    }
    ...
    if ($this->_isOldRouteMethod())
    {
      ...
      return $this->link_to($this->renderLabel(), $this->getRoute(), array_merge($this->getUrlOptions(), $this->getLinkOptions()));
    }
    else
    {
      ...
      return $this->link_to($this->renderLabel(), $this->getRoute(), $params, $options);
    }
  }
  ...
  public function _tag_options($options = array())
  {
      $options = $this->_parse_attributes($options);

      $html = '';
      foreach ($options as $key => $value) {
          $html .= ' '.$key.'="'.$this->escape_once($value).'"';
      }

      return $html;
  }

  public function _parse_attributes($string) 
  {
  ...
  {
      return $this->fix_double_escape(htmlspecialchars($html, ENT_COMPAT, 'utf-8'));
  }
  ...
  public function link_to($label, $route, $options)
  {
  ...
  public function content_tag($name, $content = '', $options = array())
  {
      ...
      return '<'.$name.$this->_tag_options($options).'>'.$content.'</'.$name.'>';
  }
}
