...
class Pagination
{
    ...
    public static function __callStatic($name, $arguments)
    {
        ...
        throw new \Exception('The pagination class doesn\'t have a method called "'.$name.'"');
    }
    ...
    public static function forge($name = 'default', $config = array())
    {
        ...
        {
            throw new \Exception('Pagination with this name exists already, cannot be overwritten.');
            return $exists;
        }
    ...
    public function pages_render()
    {
        ...
        for($i = $start; $i <= $end; $i++)
        {
            ...
            {
                $uri = $this->config['pagination_url'];
                if ($i == 1) {
                    $uri = explode('/', $this->config['pagination_url']);
                    array_pop($uri);
                    $uri = implode('/', $uri);
                }

                $url = ($i == 1) ? "?" . (isset($_SERVER['QUERY_STRING']) ? $_SERVER['QUERY_STRING'] : '')  : '/'. $i . (isset($_SERVER['QUERY_STRING']) ? "?" . $_SERVER['QUERY_STRING'] : '');

                $html .= str_replace(
                    '{link}',
                    str_replace(array('{uri}', '{page}'), array(rtrim($uri, '/').$url, $i), $this->template['regular-link']),
    ...
    public function previous($marker = '&laquo;')
    {
        ...
        if ($this->config['total_pages'] > 1)
        {
            ...
            else
            {
                ...
                $html = str_replace(
                    '{link}',
                    str_replace(array('{uri}', '{page}'), array(rtrim($uri, '/').$previous_page.$query, $marker), $this->template['previous-link']),
    ...
    public function next($marker = '&raquo;')
    {
        ...
        if ($this->config['total_pages'] > 1)
        {
            ...
            {
                $next_page = '/'.($this->config['current_page'] + 1) . (isset($_SERVER['QUERY_STRING']) ? "?" . $_SERVER['QUERY_STRING'] : '');
    ...
    protected function _recalculate()
    {
        ...
        $this->config['current_page'] = ($this->config['total_items'] > 0 and $this->config['current_page'] > 1) ? $this->config['current_page'] : (int) $this->config['uri_segment'];
